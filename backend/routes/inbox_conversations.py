...
def create_conversation(
    ...
    conversation, created = svc.get_or_create(customer_phone, None, customer_name)
...
def list_conversations(
    ...
    query = db.query(WhatsAppInboxConversation)
    if status:
...
def get_conversation(
    ...
    conv = db.query(WhatsAppInboxConversation).filter(WhatsAppInboxConversation.id == conversation_id).first()
    if not conv:
...
def get_conversation_messages(
    ...
    total = db.query(WhatsAppInboxMessage).filter(WhatsAppInboxMessage.conversation_id == conversation_id).count()
